...
class UserController extends Controller
{
    ...
    public function create()
    {
        ...
        if (auth()->user()->role === 'Operador') {
            $availableRoles = array_values(array_filter($availableRoles, function($role) {
                return in_array($role, ['Estudiante', 'Registrado', 'Docente', 'Operador', 'Auxiliar', 'Consultor Agesoc', 'Consultor Asstracud']);
            }));
        }
    ...
    public function store(Request $request)
    {
        ...
        if (auth()->user()->role === 'Operador' && !in_array($request->role, ['Estudiante', 'Registrado', 'Docente', 'Operador', 'Auxiliar', 'Consultor Agesoc', 'Consultor Asstracud'])) {
            return redirect()->back()
                ->withErrors(['role' => 'No tienes permisos para asignar ese rol. Solo se permiten Estudiante, Registrado, Docente, Operador, Auxiliar, Consultor Agesoc y Consultor Asstracud.'])
                ->withInput();
        }
